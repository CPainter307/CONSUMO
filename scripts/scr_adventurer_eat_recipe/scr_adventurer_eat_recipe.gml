...
currentHealth = min(current_recipe.hp+currentHealth, maxHealth)
if current_recipe.hp > 0 {
	...
	_h.buff_amount = string(current_recipe.hp)
	...
	_hg.buff_amount = string(current_recipe.hp)
}

//set timers
attackTimer = (attackTimer/60) + current_recipe.attack;
defenseTimer = (defenseTimer/60) + current_recipe.defense;
speedTimer = (speedTimer/60) + current_recipe.spd;
...
if (current_recipe.hp < 0 || current_recipe.attack < 0 || current_recipe.defense < 0 || current_recipe.spd < 0) {
	flash = 3;	
} else {
	shaderBuff = 1;	
}
...
// destroy recipe
with (current_recipe) instance_destroy();	