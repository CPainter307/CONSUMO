...
for (var i = value-1; i > 0 ; i -= 1) {
	...
	if ds_grid_value_exists(ds_gridpathfinding, xgoal,ygoal, xgoal,ygoal+1, i) { // check for one way platform above
		xgoal = ds_grid_value_x(ds_gridpathfinding, xgoal,ygoal, xgoal,ygoal+1,i);  // Store the X coordinate in xgoal
        ygoal = ds_grid_value_y(ds_gridpathfinding, x_previous,ygoal, x_previous,ygoal+1,i); // Store the Y coordinate in ygoal
