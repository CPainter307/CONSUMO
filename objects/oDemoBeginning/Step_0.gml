if (playing) {
	...
	trigger_inst.t_scene_info = [
		...
		[cutscene_change_variable, oDemoBeginning, "playing", false],
	]
}
...
if (keyboard_check_pressed(vk_f1)) {
	if (playing) {
		...
		instance_destroy(oAutoTrigger);
		new_trigger = instance_create_layer(0, 0, "Objects", oAutoTrigger);
		new_trigger.t_scene_info = [
			[cutscene_stop_sound, snd_epic_theme],
			[cutscene_stop_sound, snd_main_theme],
			[cutscene_instance_destroy, eFrogEgg,],
			[cutscene_instance_create, 3156, 2031, "Objects", eFrogEgg,],
			[cutscene_change_camera_target, oPlayer],
			[cutscene_change_variable, oAdventurer, "startFight", true],
			[cutscene_play_sound, snd_main_theme, 2, true],
		    [cutscene_change_variable, oDemoBeginning, "playing", false],
