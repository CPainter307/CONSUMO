...
//draw bg
//draw_set_alpha(.5)
draw_sprite_ext(sPlayerInventory, 0, inventory_x-(cell_size/2), inventory_y-(cell_size/2), 1, 1, 0, c_white, .7)
	...
	switch (ii) {	
		...
		case selected_slot:
			...
			if inv_grid[# 0, selected_slot] != 0 {
				...
				draw_set_alpha(.7)
				draw_set_color(BRIGHT_BROWN_COL)
				draw_rectangle(xx-cell_size/2, yy-cell_size/2, xx+cell_size/2, yy+cell_size/2, false)
				draw_set_alpha(1)
				if iitem != 0 {
					draw_sprite_ext(iitem[1], 0, xx, yy, item_scale, item_scale, 0, c_white, 1)
